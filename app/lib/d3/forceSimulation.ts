...
// Force Simulationの作成
export function createForceSimulation(nodes: SimulationNode[], links: SimulationLink[]) {
  const simulation = d3
    ...
    .force(
      'collision',
      d3.forceCollide<SimulationNode>().radius((d) => getNodeSize(d) / 2 + 10)
    )
    .force(
      'x',
      d3
...
function createAreaSimulation(
  ...
  return d3
    .forceSimulation<SimulationNode>(areaNodes)
    .force(
      'link',
      d3
        .forceLink<SimulationNode, SimulationLink>(areaLinks)
        .id((d) => d.id)
...
export function createKnowledgeBaseSimulation(nodes: SimulationNode[], links: SimulationLink[]) {
  return createAreaSimulation(nodes, links, 'knowledge_base')
}

// IdeaStockエリア専用のシミュレーション
export function createIdeaStockSimulation(nodes: SimulationNode[], links: SimulationLink[]) {
