...
## UIと諸概念について
Obsidianのグラフビューをそのまま実装し改変していくイメージです。ノードとエッジを用い、インタラクティブな動的表現（ドラッグやクリック、スムーズな動き、中心力・反発力の設定など）を実装したい。

空間は横線で5分割されており、上からKnowledgeBase、IdeaStock、Build、Measure、Learnと定義されます。各エリアは同一空間上、同一画面上に存在します。

ユーザーは上記の流れに沿って、上のエリアから下のエリアへと段階を進めていきます。IdeaStockからはエリアを垂直にまたがるようにノードが連なっていきますが、この垂直のまとまりをプロジェクトと呼称します。ParallelLeanでは複数プロジェクトの並列進行が前提であるため、プロジェクトは横方向に連なっていきます。

[参考画像](ParallelLean_ui.png)
